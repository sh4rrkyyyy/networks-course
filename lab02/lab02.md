...
#### Решение
Если первый пакет пришел через $N \dfrac{L}{R}$, второй пакет придет через $\dfrac{L}{R}$
от времени прихода первого, третий $-$ через $2 \dfrac{L}{R}$ от времени прихода первого и тд,
пакет с номером P $-$ через $(P - 1) \dfrac{L}{R}$ от времени прихода первого, поэтому задержка прохождения всех пакетов $-$ 
$(N + P - 1) \dfrac{L}{R}$
...
#### Решение
Пусть $size$ $-$ размер пакета, общее время передачи равно времени прохождения всех данных через первый (самый медленный) канал плюс
времени прохождения последнего пакета через второй и третий каналы, то есть общее время равно $\dfrac{5 * 2 ^ {23}}{200 * 10 ^ 3} + \dfrac{size}{3 * 10^6} + \dfrac{size}{5 * 10^6} \approx 200 + \dfrac{5}{6 * 10^6}size$
с
...
#### Решение
Т.к. пропускная способность канала равна 2 Мбит/c, максимум одновременно могут передавать 20 пользователей, тогда 
вероятность будет равна $\sum_{i=12}^{20} C_{60}^i 0.2^i 0.8^{60-i} \approx 0.55$
...
#### Решение
$\dfrac{\dfrac{X}{S} (80 + S)}{R} + 2 \dfrac{80+S}{R}= \dfrac{(\dfrac{X}{S} + 2)(80 + S)}{R}$ $-$ время передачи. Минимум выражения
достигается при S = $\sqrt{40X}$
...
#### Решение
Общая задержка равна $\dfrac{L}{R} + \dfrac{I L}{R(1 - I)} = \dfrac{L}{R(1-I)} = \dfrac{L}{R(1-(\dfrac{La}{R}))} = 
\dfrac{L}{R} \dfrac{1}{1 - \dfrac{L}{R} a}$

Чем больше $\dfrac{L}{R}$, тем больше первый множитель $\dfrac{L}{R}$ и тем меньше знаменатель второго множителя $\dfrac{1}{1 - \dfrac{L}{R} a}$ и тем самым больше второй множитель, значит
чем больше $\dfrac{L}{R}$, тем выше общая задержка
