...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - Потому что ICMP работает на сетевом уровне, а номера портов используются на транспортном уровне
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   ...
   - На каждое из полей по 2 байта
   ![err](images/request.png)
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - ICMP-пакет содержит заголовок исходного IP-пакета и ICMP-пакет, который вызвал ошибку, так же появилось новое поле Unused, заполненное нулями
...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - у этих пакетов Type: 0 (Echo (ping) reply), в отличие от пакетов сообщающих об ошибках, у которых Type: 11 (Time-to-live exceeded), в них нету исходного IP-пакета и ICMP-пакета
   - Это обычные ответы, никакой ошибки не было, т.к. значения TTL хватило, чтобы пакет дошел до конца
   ![err](images/last.png)
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - есть, 11-12 (51ms -> 123 ms)
   - 11 -- Лондон (lhr01), 12 -- Бостон (bos01)
   ![err](images/traceroute.png)
