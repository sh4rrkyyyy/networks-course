...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Host: gaia.cs.umass.edu, Connection: keep-alive (переиспользовать tcp соединение), User-Agent -- ОС, версия браузера,
   Accept -- какие форматы принимает браузер, Accept-Encoding (алгоритмы сжатия, которые поддерживает браузер)
3. Какой IP-адрес вашего компьютера? Какой адрес сервера gaia.cs.umass.edu?
   - IP-адрес моего компьютера -- 192.168.0.104
   - адрес сервера gaia.cs.umass.edu -- 128.119.245.12
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - вернул 
   - это можно увидеть по Line-based text data: text/html (10 lines)
...
4. Есть ли в передаваемых данных какая-либо информация заголовка HTTP, связанная с
   сегментацией TCP?
   - нет, т.к. http на своем уровне ничего не знает о сегментации tcp
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 4 (без учета запроса иконки 3)
   - 128.119.245.12, 128.119.245.12, 2.56.99.24, 128.119.245.12
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - параллельно, запрос GET на второе изображение отправлен раньше, чем получен ответ на GET для первого изображения
